...
function BD.monster_setup(self)
  for name,opt in pairs(self.options) do
    ...
    if M and opt.value != "default" then
      M.prob    = BD.MON_PROBS[opt.value]
      M.density = BD.DENSITIES[opt.value]
...
function BD.weapon_setup(self)
  for name,opt in pairs(self.options) do
    ...
    if W and opt.value != "default" then
      W.add_prob = BD.WEAPON_PROBS[opt.value]
      W.pref     = BD.WEAPON_PREFS[opt.value]
